...
def batchnorm_forward(x, gamma, beta, bn_param):
  ...
  if mode == 'train':
    ...
    running_var = momentum * running_var + (1 - momentum) * sample_var
    x_u = x - sample_mean
    x_sv = np.sqrt(sample_var + eps)
    x_iv = 1/x_sv
    x_n = x_u * x_iv
    out = x_n * gamma + beta
    cache = (x_u, x_n, x_sv, x_iv, gamma, eps)
    ...
    x_tn = (x - running_mean)/np.sqrt(running_var + eps)
    out = x_tn * gamma + beta
...
def batchnorm_backward(dout, cache):
  ...
  x_u, x_n, x_sv, x_iv, gamma, eps = cache
  N, _ = dout.shape
  ...
  dx = dout * gamma
  div = np.sum(x_u * dx, axis=0)
  da = dx * x_iv
  dsv =  -1 / (np.square(x_sv)) * div
  dv = 0.5 * 1 / x_sv * dsv
  dvar = 2 * x_u / N * dv
  dmean = - x_iv * np.sum(dx, axis=0) / N
