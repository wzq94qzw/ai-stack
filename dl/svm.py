...
class SVM(object):
  ...
  def loss(self, X, y):
    ...
    # ================================================================ #
      ...
      for j in np.arange(num_classes):
        if j != y[i]:
          l_i = 1 + self.W[j].dot(X[i]) - self.W[y[i]].dot(X[i])
          if l_i > 0:
            loss_tmp += l_i
  ...
  def loss_and_grad(self, X, y):
    ...
    # ================================================================ #
      ...
      grad_tmp[y[i]] -= np.sum(grad_tmp, axis=0)
  ...
  def fast_loss_and_grad(self, X, y):
    ...
    loss_vector = loss_matrix[np.arange(loss_matrix.shape[0]), y].reshape(-1, 1)

    loss_hinge = np.maximum(loss_matrix - loss_vector + 1.0, 0.0) #NxC
    loss_hinge[np.arange(loss_matrix.shape[0]), y] = 0
    loss += np.sum(loss_hinge)/X.shape[0]
    ...
    loss_hinge[loss_hinge > 0] = 1
    loss_hinge[np.arange(loss_matrix.shape[0]), y] = -1 * np.sum(loss_hinge, axis=1)
    grad = np.matmul(loss_hinge.T, X)
    grad /= X.shape[0] 
