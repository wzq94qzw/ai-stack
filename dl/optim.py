...
def adam(w, dw, config=None):
  ...
  v_bc = 1 / (1 - np.power(config['beta1'], config['t'])) * config['v']
  a_bc = 1 / (1 - np.power(config['beta2'], config['t'])) * config['a']
  next_w = w - config['learning_rate'] / (np.sqrt(a_bc) + config['epsilon']) * v_bc
